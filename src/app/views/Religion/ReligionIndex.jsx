...
export default observer(function ReligionIndex() {
  ...
  const { religionList, currentReligion } = religionStore;
  ...
  const search = useFormik({
    ...
    onSubmit: (values) => {
      religionStore.pagingReligionsAsync(page, rowsPerPage, values.keyword);
      setPage(0);
    }
  });
  ...
  function handleAddBtn() {
    religionStore.clearCurrentReligion();
    setIsShowModal(true);
    setType("new");
  }

  function handleEditBtn({ id }) {
    religionStore.getReligionAsync(id);
    setIsShowModal(true);
    setType("edit");
  }
  ...
  function handleAgreeBtn() {
    religionStore.deleteReligionAsync(currentReligion.id).then(() => {
      handleUpdateTable();
    });
    setOpenDialog(false);
  }
  ...
  function handleUpdateTable() {
    page === 0 ? religionStore.pagingReligionsAsync(page, rowsPerPage, search.values.keyword) : setPage(0);
  }

  useEffect(() => {
    religionStore.pagingReligionsAsync(page, rowsPerPage, search.values.keyword);
  }, [page, rowsPerPage]);
  ...
  return (
    <>
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Delete religion"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            This action can't be undone!
            Do you want to delete this religion?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <TablePagination
        component="div"
        count={religionStore.totalReligions}
        page={page}
